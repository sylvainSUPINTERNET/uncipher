...
# checking parameters for dictionnary
if len(sys.argv) < 2:
    print("Entrer un dictionnaire en paramètre !")
    exit()
else:
    dico_file_name = sys.argv[1]
    if os.path.isfile(dico_file_name):
        # Open dict and read
        lines = list(open(sys.argv[1], encoding='utf-8').read().splitlines())
    else:
        print("Votre dictionnaire n'a pas ete trouvé!")
        exit()
...
if (arg_status is True):
    ...
    for word in words:
        ...
        for turn, shift in enumerate(range(1, 27)):
            ...
            possibilties.append(
                "".join([alphabet[alphabet.index(x) + shift] if x in alphabet else x for x in word.lower()]))
            ...
            if word_found_count == 0 and turn == 24:
                ...
                for word_dico in motResult[1]:
                    ...
                    if count_char_word_dico.most_common()[0] == count_char_word_input.most_common()[
                        0]:  # 0 return the character most common in the str
